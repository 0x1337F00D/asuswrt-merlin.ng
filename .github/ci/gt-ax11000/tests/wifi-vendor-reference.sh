...
if [[ -n ${WIFI_CANDIDATE_ROOTFS:-} ]]; then
  : "${WIFI_CANDIDATE_SHARED_SHA256:?Bind the candidate shared library explicitly}"
  candidate=$(realpath "$WIFI_CANDIDATE_ROOTFS")
  [[ $(sha256sum "$candidate/usr/lib/libshared.so" | cut -d ' ' -f 1) == "$WIFI_CANDIDATE_SHARED_SHA256" ]]
  # Test the same unmodified vendor daemon with the permanent ABI repair,
  # without the compatibility adapter. Use the candidate's complete libs.
  [[ $(sha256sum "$candidate/usr/sbin/bsd" | cut -d ' ' -f 1) == af4f653e017e8daf27058bcb2b28d2f80acbae034456c668ad7613c48b818f84 ]]
  mv "$trial/lib" "$trial/reference-lib"
  mv "$trial/usr/lib" "$trial/usr/reference-lib"
  cp -a "$candidate/lib" "$trial/lib"
  cp -a "$candidate/usr/lib" "$trial/usr/lib"
  run_case candidate "$trial/fixture.so" 124
  ! rg -qi 'segmentation fault|uncaught target signal|core dumped' "$trial/candidate.log"
  rg -q 'Tri-Band Smart Connect' "$trial/candidate.log"
  for radio in eth6 eth7 eth8; do rg -q "fixture wl_ioctl $radio 105" "$trial/candidate.log"; done
  echo "BSD_CANDIDATE_ABI=PASS adapter=absent shared_sha256=$WIFI_CANDIDATE_SHARED_SHA256"
fi
