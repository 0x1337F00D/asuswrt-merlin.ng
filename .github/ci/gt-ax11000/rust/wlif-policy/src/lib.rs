...
///
/// Valid UTF-8 is also required, which is stricter than this profile:
/// `config_base` says `# RTCONFIG_UTF8_SSID is not set`, so 802.11 permits
/// any 32 bytes here.  A legitimately non-UTF-8 SSID is therefore refused.
/// Nothing ships that consequence today because every caller of this function
/// is compiled out on GT-AX11000, and `DEBTS_AND_TODOS.md` records that the
/// rule has to be revisited before any of them becomes live.
#[must_use]
